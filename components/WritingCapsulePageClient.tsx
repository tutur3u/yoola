...
import type { ArchiveArtwork, LoreCapsule } from "@/lib/archive-data";
...
export default function WritingCapsulePageClient({ slug }: { slug: string }) {
  ...
  const capsule = loreCapsules.find((item) => item.slug === slug) ?? null;
  const artwork = capsule?.artworkId
    ? (artworks.find((item) => item.id === capsule.artworkId) ?? null)
    : null;
  const writingSection = archiveQuery.data?.sections.writing ?? null;
        ...
        <div className="file-frame border border-white/10 bg-black/72 p-6 backdrop-blur md:p-8">
          <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_320px]">
            <div className="space-y-8">
              ...
              <div className="border-l-2 border-[#b026ff] pl-4">
                <p className="font-mono text-[11px] uppercase tracking-[0.35em] text-[#ff72c9]">
                  Description
                </p>
                <div className="mt-4">
            ...
            <div className="space-y-6">
              {artwork ? (
                <div className="relative overflow-hidden border border-white/10 bg-black/60 p-4">
